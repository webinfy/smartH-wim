angular.module('AceApp')
        ...
        .controller('MainController', function ($scope, $rootScope, $http, $timeout, $location/**, cfpLoadingBar*/) {
            ...
            $rootScope.getData = function (dataName, type) {
                ...
                if (!dataPath)
                    return;
                if (dataKey in $rootScope.appData)
                    return $rootScope.appData[dataKey];
...
angular.module('AceApp').controller('SettingsCtrl', function ($scope, $timeout, SAVE_SETTING, $localStorage, StorageGet) {
    ...
    if (SAVE_SETTING)
        $localStorage['ace.settings'] = $localStorage['ace.settings'] || {};
    ...
    if (SAVE_SETTING)
        StorageGet.load($scope, 'ace.settings');//load previously saved setting values
    ...
    $scope.$watch('ace.settings.navbar', function (newValue) {
        ...
        $timeout(function () {
            if (jQuery)
                jQuery(document).trigger('settings.ace', ['navbar_fixed', newValue]);
        });

        if (SAVE_SETTING)
            $localStorage['ace.settings']['navbar'] = newValue;
    });
    $scope.$watch('ace.settings.sidebar', function (newValue) {
        ...
        $timeout(function () {
            if (jQuery)
                jQuery(document).trigger('settings.ace', ['sidebar_fixed', newValue]);
        });

        if (SAVE_SETTING)
            $localStorage['ace.settings']['sidebar'] = newValue;
    });
    $scope.$watch('ace.settings.breadcrumbs', function (newValue) {
        ...
        $timeout(function () {
            if (jQuery)
                jQuery(document).trigger('settings.ace', ['breadcrumbs_fixed', newValue]);
        });

        if (SAVE_SETTING)
            $localStorage['ace.settings']['breadcrumbs'] = newValue;
    });
    $scope.$watch('ace.settings.container', function (newValue) {
        $timeout(function () {
            if (jQuery)
                jQuery(document).trigger('settings.ace', ['main_container_fixed', newValue]);
        });

        if (SAVE_SETTING)
            $localStorage['ace.settings']['container'] = newValue;
    });
    ...
    $scope.$watch('ace.settings.compact', function (newValue) {
        ...
        if (SAVE_SETTING)
            $localStorage['ace.settings']['compact'] = newValue;
    });
    $scope.$watch('ace.settings.hover', function (newValue) {
        ...
        if (SAVE_SETTING)
            $localStorage['ace.settings']['hover'] = newValue;
    });
    $scope.$watch('ace.settings.highlight', function (newValue) {
        if (SAVE_SETTING)
            $localStorage['ace.settings']['highlight'] = newValue;
    });
    ...
    $scope.$watch('ace.settings.skinIndex', function (newValue) {
        ...
        if (SAVE_SETTING)
            $localStorage['ace.settings']['skinIndex'] = newValue;
    });
...
app.controller('SidebarCtrl', function ($scope, $state, $rootScope, $timeout, SidebarList, SAVE_SETTING, $localStorage, StorageGet) {
    ...
    $rootScope.isSubOpen = function (name) {
        if (!(name in $rootScope.subMenuOpen))
            $rootScope.subMenuOpen[name] = false;
